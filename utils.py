...
def get_images_by_dir(dirname):
    img_names = os.listdir(dirname)  # listdir 返回文件夹中文件的名字
    img_paths = [dirname + '/' + img_name for img_name in img_names]
    imgs = [cv2.imread(path) for path in img_paths]
    return imgs  # 返回指定目录的所有的图像
...
def get_M_Minv():
    ...
    M = cv2.getPerspectiveTransform(src, dst)  # 从四对对应的点计算一个透视变换
...
# returns the undistorted image   对图片进行摆正
...
'''
该函数用cv2的sobel边缘检测计算x轴和y轴方向的颜色变化导数，在进行阈值过滤得到二进制图
'''
